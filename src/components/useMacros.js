...
export const useMacros = (initialLunch = null, initialDinner = null) => {
  ...
  const getBaseMacros = useCallback((cls) =>
    cls === 'Dinner' ? dinnerMacros : lunchMacros,
  [lunchMacros, dinnerMacros]);

  const updateLunchMacro  = useCallback((field, value) =>
    setLunchMacros(prev  => ({ ...prev, [field]: value })), []);
  const updateDinnerMacro = useCallback((field, value) =>
    setDinnerMacros(prev => ({ ...prev, [field]: value })), []);

  const updateDayMacro = useCallback((day, cls, field, value) => {
    setDayMacros(prev => {
  ...
  const getEffectiveMacros = useCallback((day, cls) =>
    dayMacros?.[day]?.[cls] ?? (cls === 'Dinner' ? dinnerMacros : lunchMacros),
  [dayMacros, lunchMacros, dinnerMacros]);

  const isDayOverridden = useCallback((day, cls) =>
    dayMacros?.[day]?.[cls] != null,
  [dayMacros]);

  return {
    lunchMacros,  setLunchMacros,
    dinnerMacros, setDinnerMacros,
    dayMacros,
    updateLunchMacro,
    updateDinnerMacro,
    updateDayMacro,
