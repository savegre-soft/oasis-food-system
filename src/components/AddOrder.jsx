...
const AddOrder = ({ onSuccess }) => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      const { data } = await supabase
        ...
        .from('clients')
        .select(
          `id_client, name, client_type, lunch_macro:macro_profiles!clients_lunch_macro_profile_id_fkey(id_macro_profile,name,protein_value,protein_unit,carb_value,carb_unit), dinner_macro:macro_profiles!clients_dinner_macro_profile_id_fkey(id_macro_profile,name,protein_value,protein_unit,carb_value,carb_unit)`
        )
        .order('name');
      setClients(data ?? []);
    };
  ...
  useEffect(() => {
    ...
    if (lm)
      setLunchMacros({
        protein_value: lm.protein_value,
        protein_unit: lm.protein_unit,
        carb_value: lm.carb_value,
        carb_unit: lm.carb_unit,
      });
    if (dm)
      setDinnerMacros({
        protein_value: dm.protein_value,
        protein_unit: dm.protein_unit,
        carb_value: dm.carb_value,
        carb_unit: dm.carb_unit,
      });
  }, [selectedClient]);
  ...
  useEffect(() => {
    ...
    if (macro) {
      setExpressMacros({
        protein_value: macro.protein_value,
        protein_unit: macro.protein_unit,
        carb_value: macro.carb_value,
        carb_unit: macro.carb_unit,
      });
    } else {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      const { data } = await supabase
        ...
        .select(
          'id_template, name, meal_type, order_template_details(recipe_id, quantity, recipes(id_recipe, name))'
        )
        .eq('meal_type', 'Family')
        .eq('is_active', true);
  ...
  useEffect(() => {
    ...
    const templates = [];
    if (menuType === 'Lunch' || menuType === 'both')
      if (selectedLunchTemplate) templates.push({ tmpl: selectedLunchTemplate, type: 'Lunch' });
    if (menuType === 'Dinner' || menuType === 'both')
      if (selectedDinnerTemplate) templates.push({ tmpl: selectedDinnerTemplate, type: 'Dinner' });
  ...
  const handleSubmit = async () => {
    ...
    for (const type of menuTypes) {
        ...
        .insert([
          {
            ...
            protein_snapshot: isExpress
              ...
              : ((type === 'Dinner' ? dinnerMacros?.protein_value : lunchMacros?.protein_value) ??
                null),
            protein_unit_snapshot: isExpress
              ? expressMacros.protein_unit
              : ((type === 'Dinner' ? dinnerMacros?.protein_unit : lunchMacros?.protein_unit) ??
                null),
            carb_snapshot: isExpress
              ? expressMacros.carb_value
              : ((type === 'Dinner' ? dinnerMacros?.carb_value : lunchMacros?.carb_value) ?? null),
            carb_unit_snapshot: isExpress
              ? expressMacros.carb_unit
      ...
      for (const day of activeDays) {
        ...
        const { data: detData, error: detErr } = await supabase
          ...
          .insert(
            detailsWithIdx.map(({ r, origIdx }, i) => {
    ...
    if (createdOrderIds.length > 0) {
      if (associatePaymentId) {
        ...
        const links = createdOrderIds.map((oid) => ({
          payment_id: associatePaymentId,
          order_id: oid,
        }));
        ...
        {stepLabels.map((label, i) => {
          ...
          return (
            <div key={label} className="flex items-center gap-2 flex-1 last:flex-none">
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition ${active ? 'bg-slate-800 text-white' : done ? 'bg-green-500 text-white' : 'bg-slate-100 text-slate-400'}`}
              >
                {done ? '✓' : pos}
              </div>
              <span
                className={`text-xs font-medium hidden sm:block ${active ? 'text-slate-800' : 'text-slate-400'}`}
              >
                {label}
              </span>
        ...
        <div className="space-y-4">
            ...
            {clients
              ...
              .map((c) => (
                ...
                >
                  ...
                  <span
                    className={`ml-2 text-xs ${selectedClient?.id_client === c.id_client ? 'text-slate-300' : 'text-slate-400'}`}
                  >
          ...
          {selectedClient && !familyClient && (
            ...
            >
              ...
              <div
                className={`flex flex-col items-end text-xs ${isExpress ? 'text-amber-600' : 'text-slate-400'}`}
              >
                <span>{isExpress ? 'Activo — entrega hoy' : 'Entrega según ruta'}</span>
                {isExpress && (
                  <span className="text-[10px] opacity-70">Sin plantilla ni ruta fija</span>
                )}
              </div>
            </button>
          )}
                ...
                <div key={idx} className="space-y-1">
                  ...
                  {item.recipe_id && (
                    <RecipeIngredientEditor
                      ...
                      baseIngredients={
                        recipeIngredients[String(item.recipe_id)] ?? {
                          protein: [],
                          carb: [],
                          extra: [],
                        }
          ...
          {paymentType === 'monthly' && availableMonthly.length > 0 && (
            <div className="border border-violet-200 bg-violet-50 rounded-2xl p-4 space-y-2">
              ...
              <p className="text-xs text-violet-600">
                Este cliente tiene pagos mensuales con espacio disponible. Puedes asociar esta orden
                a uno existente en lugar de crear un pago nuevo.
              </p>
                ...
                {availableMonthly.map((mp) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`ml-2 text-xs ${isSelected ? 'text-violet-200' : 'text-slate-400'}`}
                      >
      ...
      {step === 5 && (
        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${familyClient ? 'bg-purple-50 text-purple-700' : 'bg-blue-50 text-blue-700'}`}
            >
          ...
          <div>
            ...
            <div className="space-y-2">
              {DAYS_ORDER.filter((d) => (dayRecipes[d] ?? []).some((r) => r.recipe_id)).map(
                (day) => (
