...
import {
  DAYS_ORDER, DAY_LABELS, isFamily,
  getDateForDay,
} from './orderUtils';
...
const EditOrder = ({ order, onSuccess }) => {
  ...
  const {
    ...
    setOverride, toggleDay, loadFromOrderDays,
  } = useDayRecipes();
  ...
  useEffect(() => {
    ...
    if (isLunch) setLunchMacros({ protein_value: order.protein_snapshot ?? '', protein_unit: order.protein_unit_snapshot ?? 'g', carb_value: order.carb_snapshot ?? '', carb_unit: order.carb_unit_snapshot ?? 'g' });
    else         setDinnerMacros({ protein_value: order.protein_snapshot ?? '', protein_unit: order.protein_unit_snapshot ?? 'g', carb_value: order.carb_snapshot ?? '', carb_unit: order.carb_unit_snapshot ?? 'g' });
  ...
  const handleSubmit = async () => {
    ...
    const { error: orderErr } = await supabase.schema('operations').from('orders').update({
    ...
    if (orderErr) { sileo.error('Error al actualizar el pedido'); console.error(orderErr); setLoading(false); return; }

    // Delete existing order_days (cascades to details + overrides)
    const { error: delErr } = await supabase.schema('operations').from('order_days')
      .delete().eq('order_id', order.id_order);
    if (delErr) { sileo.error('Error al limpiar días'); console.error(delErr); setLoading(false); return; }

    // Re-insert order_days + details
    for (const day of activeDays) {
      const { data: dayData, error: dayErr } = await supabase.schema('operations').from('order_days')
        .insert([{ order_id: order.id_order, day_of_week: day, delivery_date: getDateForDay(day, weekStart, routeDays), status: 'PENDING' }])
        .select('id_order_day').single();
      if (dayErr) { sileo.error(`Error en día ${DAY_LABELS[day]}`); console.error(dayErr); setLoading(false); return; }
      ...
      const { data: detData, error: detErr } = await supabase.schema('operations').from('order_day_details')
        .insert(details.map((r) => {
          ...
          return { order_day_id: dayData.id_order_day, recipe_id: r.recipe_id, quantity: Number(r.quantity) || 1, protein_value_applied: eff?.protein_value ?? null, protein_unit_applied: eff?.protein_unit ?? null, carb_value_applied: eff?.carb_value ?? null, carb_unit_applied: eff?.carb_unit ?? null };
        }))
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            {order.clients?.name} · {new Date(order.week_start_date + 'T00:00:00').toLocaleDateString('es-CR', { day: '2-digit', month: 'long' })} — {new Date(order.week_end_date + 'T00:00:00').toLocaleDateString('es-CR', { day: '2-digit', month: 'long', year: 'numeric' })}
          </p>
        </div>
      </div>
