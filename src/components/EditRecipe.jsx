...
const EditRecipe = ({ recipe, onSuccess }) => {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      const fileName = Date.now() + '-' + recipe.id_recipe + '.' + fileExt;
  ...
  const save = async () => {
    ...
    try {
      ...
      const existingIds = recipe.recipe_ingredients?.map((i) => i.id_recipe_ingredient) ?? [];

      const currentIds = ingredients.filter((i) => typeof i.id === 'number').map((i) => i.id);
      ...
      for (const ing of ingredients) {
        ...
        if (typeof ing.id === 'number') {
          ...
          await supabase.schema('operations').from('recipe_ingredients').insert({
  ...
  return (
    <div className="bg-slate-50 p-8 flex justify-center">
      <div className="w-full max-w-xl bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
        ...
        {/* Imagen */}
        <div className="mb-6">
          <p className="text-sm font-medium text-slate-600 mb-2">Imagen del plato</p>
