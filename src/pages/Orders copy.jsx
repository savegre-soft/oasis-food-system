        ...
        <div className="flex items-start justify-between p-5 border-b border-slate-100">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span
                className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${isFamilyClient ? 'bg-purple-50 text-purple-700' : 'bg-blue-50 text-blue-700'}`}
              >
                {isFamilyClient ? (
                  <>
                      ...
                      <div className="flex-1 min-w-0">
                          ...
                          <div className="space-y-0.5">
                            {details.map((det, i) => (
                              <p key={i} className="text-xs text-slate-600">
                                {det.recipes?.name ?? '—'}
                                {det.quantity > 1 && (
                                  <span className="text-slate-400"> ×{det.quantity}</span>
                                )}
                              </p>
                            ))}
                          </div>
                        ...
                        {od.delivery_date && (
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            ...
                            <span
                              className={`ml-2 px-1.5 py-0.5 rounded-full border text-[10px] ${daySt.cls}`}
                            >
                              {daySt.label}
                            </span>
                          </p>
          ...
          {TABS.map(({ id, label, Icon }) => {
            ...
            return (
              ...
              >
                ...
                {count > 0 && (
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded-full font-semibold ${active ? 'bg-slate-600 text-slate-200' : 'bg-slate-100 text-slate-500'}`}
                  >
                    {count}
                  </span>
                )}
