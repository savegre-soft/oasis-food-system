...
import { ClipboardList, Calendar, History, ChevronLeft, ChevronRight, X, User, Users } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-start justify-between p-5 border-b border-slate-100">
          ...
          <button onClick={onClose} className="p-1.5 rounded-xl hover:bg-slate-100 transition text-slate-400 hover:text-slate-600 shrink-0 ml-3">
            <X size={18} />
          </button>
        </div>
...
const OrderCard = ({ order, onClick }) => {
  ...
  return (
    <button
      ...
      className="w-full text-left bg-white p-5 rounded-2xl shadow-sm border border-slate-100 hover:border-slate-300 hover:shadow-md transition"
    >
...
const HistoryView = ({ orders, onOrderClick }) => {
          ...
          <div key={weekStart}>
            ...
            <div className="space-y-3">
              {weekOrders.map((order) => (
                <OrderCard key={order.id_order} order={order} onClick={onOrderClick} />
              ))}
            </div>
          </div>
        );
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'week' && (
              <motion.div key="week" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }}>
                ...
                {weekOrders.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {weekOrders.map((order) => (
                      <OrderCard key={order.id_order} order={order} onClick={setSelectedOrder} />
                    ))}
                  </div>
                )}
              </motion.div>
            ...
            {activeTab === 'history' && (
              <motion.div key="history" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }}>
                <HistoryView orders={historyOrders} onOrderClick={setSelectedOrder} />
              </motion.div>
            )}
