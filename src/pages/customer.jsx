...
const CLIENT_TYPE = {
  personal: { label: 'Personal', className: 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' },
  family: { label: 'Familiar', className: 'bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400' },
};

const PLAN_TYPE = {
  estandar: { label: '⭐ Estándar', className: 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300' },
  nutricional: { label: '🥗 Nutricional', className: 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400' },
};
...
const Customer = () => {
  ...
  if (loading) return <p className="p-8 text-slate-400 dark:text-slate-500">Cargando cliente...</p>;
  if (!customer) return <p className="p-8 text-slate-400 dark:text-slate-500">Cliente no encontrado</p>;
      ...
      <div className="p-8 max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
            ...
            <div className="space-y-1 text-sm text-slate-700 dark:text-slate-300 md:text-right">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-1">Contacto</p>
        ...
        {/* Portal de cliente */}
        <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
          <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-3 flex items-center gap-1.5">
            <Link2 size={13} /> Portal de cliente
          </p>
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            <p className="flex-1 text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-xl px-3 py-2.5 truncate">
              {window.location.origin}/portal/{customer.portal_token}
            </p>
            <div className="flex gap-2 shrink-0">
              <button
                onClick={handleCopyLink}
                className="flex items-center gap-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2.5 rounded-xl hover:border-slate-400 dark:hover:border-slate-500 transition"
              >
                <Copy size={13} /> Copiar enlace
              </button>
              <button
                onClick={() => setConfirmingRegen(true)}
                className="flex items-center gap-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2.5 rounded-xl hover:border-amber-400 dark:hover:border-amber-500 hover:text-amber-700 dark:hover:text-amber-400 transition"
              >
        ...
        {customer.client_type === 'personal' && (
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm space-y-3">
            ...
            {hasMacros ? (
              ...
              <p className="text-sm text-slate-400 dark:text-slate-500 italic">Sin perfil nutricional registrado</p>
            )}
          </div>
        )}
        ...
        {/* Map */}
        {hasMap && (
          <div className="bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-2xl p-6 shadow-sm space-y-3">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Ubicación</p>
